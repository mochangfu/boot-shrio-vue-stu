...
<script>
    ...
    beforeAvatarUpload(file)
    {
        ...
        if (!isLt2M) {
            this.$message.error('上传头像图片大小不能超过 2MB1!');
        }
    ...
    initUserInfo()
    {
        ...
        if (usr) {
            _this.nickName = usr.username,
                _this.userform.photoUrl = usr.photoUrl,
                _this.imgUrl = _this.fileServerIp + 'user/headerImg/' + _this.userform.photoUrl
        } else {
            _this.$router.push('/login');
        }
    }
